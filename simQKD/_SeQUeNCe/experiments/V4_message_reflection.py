"""
V4: Message Reflection - SeQUeNCe

Runs Scenario D (PM-QKD with active Charlie) to demonstrate the V4 attack.
"""
...
def main():
    print_experiment_header("V4: Message Reflection", "SeQUeNCe")
    ...
    if not SEQUENCE_AVAILABLE:
        ...
        print_result_verdict(results, 4, "SeQUeNCe", symbol=SYMBOL_UNSUPPORTED)
        return

    # Test 1: MAC and authentication check
    print("\n--- Test 1: MAC and authentication check ---")
    results["notes"].append(
        "SeQUeNCe has no built-in MAC or authentication. "
    ...
    if attack["bob_accepts_session"]:
        ...
        results["notes"].append(
            "V4 succeeds: Charlie reflects Bob's PE message. Bob verifies "
            "MAC (no directionality), sees QBER=0 (his own data reflected), "
            "and accepts session key from Charlie's injected qubits. "
            "SeQUeNCe provides simulation context; numpy handles quantum ops."
        )
    ...
    print_result_verdict(results, 4, "SeQUeNCe", symbol=symbol)
